...
- `kappa_bundle` from `c_bundle_raw * bundle_density_raw - e_bundle_raw`
- `sigma_triviality` from `triviality_floor_raw - torsor_loss_raw - restart_loss_raw`
- `kappa_compact` from `1.0 / (1.0 + delta_comp_sup_raw)`
- `rho_rigidity` from `rho_rigidity_raw`
- `torsor_transfer` from `c_transfer_raw * transfer_gain_raw - e_transfer_raw`
