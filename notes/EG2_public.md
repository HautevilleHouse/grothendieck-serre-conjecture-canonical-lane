...
## Goal
Expand the compressed capture/restart language into the local-to-global transport gate for `proving triviality of rationally trivial principal bundles in the declared admissible class through an admissible principal-bundle closure architecture`.

## Objects

- transport carrier: the admissible evolution, deformation, or routed lattice declared in the preprint.
- capture floor: `sigma_triviality`.
...
`GS_G2` closes when `sigma_triviality` survives admissible losses and restart corrections: triviality defect stays above capture floor across admissible torsor losses.
This is the transport contribution to `M_GS`.
...
Payload: verify that the capture constant `sigma_triviality` is present in the constants registry and extraction inputs.
...
Payload: inspect the repro script and guard output for the gate tied to `sigma_triviality`.

### Theorem EG2.3 (capture gate closure)
If transport accounting and restart preservation hold, then `GS_G2` carries local control forward without breaking admissibility.
