...
In-paper anchor: `paper/GROTHENDIECK_SERRE_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving triviality of rationally trivial principal bundles in the declared admissible class through an admissible principal-bundle closure architecture`.
...
2. the endpoint-transfer lock `torsor_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_GS` remains positive.
...
Payload: verify `torsor_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_GS` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `torsor_transfer`
