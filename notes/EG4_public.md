...
## Goal
Separate the rigidity job from the endpoint-transfer job for `proving triviality of rationally trivial principal bundles in the declared admissible class through an admissible principal-bundle closure architecture`.
...
- endpoint-transfer lock: `torsor_transfer`.
- coherence interface: `eps_coh` remains available to the bridge and final margin.

## Closure Criterion

`GS_G4` closes when `rho_rigidity` excludes bad endpoint alternatives: bad principal-bundle countermodels are excluded.
`GS_G5` closes when `torsor_transfer` transfers the surviving endpoint to the intended target class: rigid limit transfers to the Grothendieck-Serre endpoint class.
Together they feed the strict margin `M_GS`.
...
The surviving rigid representative is locked to the standard problem-side endpoint by `torsor_transfer`.

Payload: read this note together with `notes/IDENTIFICATION_BRIDGE.md`.

### Theorem EG4.3 (rigidity/transfer gate closure)
If bad limits are excluded and the endpoint lock is active, then `GS_G4` and `GS_G5` deliver the boundary object needed by the final margin.
