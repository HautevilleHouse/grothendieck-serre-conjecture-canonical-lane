...
In-paper anchor: `paper/GROTHENDIECK_SERRE_CONJECTURE_PREPRINT.md` (`GS_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving triviality of rationally trivial principal bundles in the declared admissible class through an admissible principal-bundle closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`GS_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_GS`.
...
If near-failure extraction and no-Zeno spacing hold, then `GS_G3` prevents hidden drift before rigidity is applied.
